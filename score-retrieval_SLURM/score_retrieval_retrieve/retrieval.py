...
def run_retrieval(alg_name, query_paths=query_paths, database_paths=database_paths, metric_name=DEFAULT_METRIC, debug=False):
    ...
    mrrs = []
    aps = []
    outdir = "/data1/dyang/Projects/Evan_CNNRetrieval/score-retrieval/data/results/"
    ...
    for i, (correct_label_str, veclist) in enumerate(zip(q_label_strs, q_veclists)):
        ...
        results = [label_set[i] for i in sorted_labels]
        with open(outdir+correct_label_str+".pkl","wb") as f:
