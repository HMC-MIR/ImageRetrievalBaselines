...
def get_label(image_path, query = False):
    """Get the label for the given image."""
    if (ORIGINAL_ORG and not query):
...
def get_name_dpi_ind(img_path):
    ...
    if ORIGINAL_ORG:
        ...
        ind_arr = os.path.splitext(os.path.basename(img_path))[0].split("_benchmark")[0].split("-")
        if len(ind_arr)==2:
            ind = ind_arr[1]
        else:
            ind = 0
...
def index_images(dataset=None, retrieval = False):
    """Return an iterator of (label, path) for all images."""
    data_dir = get_dataset_dir(dataset)
    print("DATADIR",data_dir)
    for dirpath, _, filenames in os.walk(data_dir):
        for fname in filenames:
            if os.path.splitext(fname)[-1] == ".jpg" or os.path.splitext(fname)[-1] == ".png" or (retrieval and os.path.splitext(fname)[-1]==".npy"):
                ...
                if QUERY_TOGGLE:
                    if TRAIN_QUERIES:
                        if int(name[1:].split("_")[0])%10 !=1 and int(name[1:].split("_")[0])%10!=5:
                            continue
                    else:
                        if int(name[1:].split("_")[0])%10 == 1 or int(name[1:].split("_")[0])%10 == 5:
                            continue
                if(QUERY_TOGGLE):
                    print(fname)
                    yield os.path.splitext(fname)[0],img_path
                else:
                    print(name,ind,get_label(img_path))
...
def index_by_label_and_name(dataset=None, sort=False, retrieval = False):
    """Run gen_label_name_index on the given dataset."""
    return gen_label_name_index(index_images(dataset,retrieval), sort=sort)
...
def gen_multi_dataset_data(
    ...
    print(query_paths,query_labels)
    # generate db data
    db_label_name_index = index_by_label_and_name(db_dataset, retrieval = True)
